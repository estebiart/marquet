...
const InternaSection: React.FC<DestacadosProps> = ({ noticias }) => {

    const handleClick = (categoria: string, iddado: number, slug: string,) => {
        ...
        <>
            {
                noticias.length >= 4 && (
                    <section className="mx-auto" >
                        <h3 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight border-l-4 border-headerColor pl-3 mb-2">
                            Actualidad
                        </h3>
                        {noticias.map((noticia, index) => (
                            <div key={index} className="max-w-4xl mx-auto overflow-hidden mb-4 border-b border-gray-300"
                                onClick={() => handleClick(noticia?.categorias[0]?.nombre, noticia.id, noticia.slug)}
